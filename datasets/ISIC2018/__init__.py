...
task3_img = 'Training'
task3_resized_img = 'Training_Resize'
task3_validation_img = 'Validation'
task3_val_resized_img = 'Validation_Resize'
task3_test_img = 'Test'
task3_test_resized_img = 'Test_Resize45'

# task3_gt = 'ISIC2018_Task3_Training_GroundTruth'
# task3_val_gt = 'ISIC2018_Task3_Validation_GroundTruth'
# task3_test_gt = 'ISIC2018_Task3_Test_GroundTruth'

MEL = 0  # Melanoma
SK = 1  # Melanocytic nevus
BN = 2

classes = [MEL, SK, BN]
class_names = ['MEL', 'SK', 'BN']
# classes = [0, 1]
# class_names = ['MM', 'Other']
...
task3_resized_img_dir = os.path.join(data_dir, task3_resized_img)
task3_validation_img_dir = os.path.join(data_dir, task3_validation_img)
task3_validation_resized_img_dir = os.path.join(data_dir, task3_val_resized_img)
task3_test_img_dir = os.path.join(data_dir, task3_test_img)
task3_test_resized_img_dir = os.path.join(data_dir, task3_test_resized_img)
...
# task3_val_gt_dir = os.path.join(data_dir, 'Validation_GroundTruth.csv')
...
if os.path.isdir(task3_validation_resized_img_dir):
    task3_validation_image_ids = [fname.rsplit('.', maxsplit=1)[0] for fname in os.listdir(task3_validation_resized_img_dir)
                                  if fname.startswith('ISIC') and fname.lower().endswith('.jpg')]
    task3_validation_image_ids.sort()

task3_test_image_ids = list()
if os.path.isdir(task3_test_resized_img_dir):
    task3_test_image_ids = [fname.rsplit('.', maxsplit=1)[0] for fname in os.listdir(task3_test_resized_img_dir)
                                  if fname.startswith('ISIC') and fname.lower().endswith('.jpg')]
    task3_test_image_ids.sort()

task3_gt_fname = 'Training_GroundTruth.csv'
# task3_sup_fname = 'ISIC2018_Task3_Training_LesionGroupings.csv'
task3_val_gt_fname = 'Validation_GroundTruth.csv'
task3_test_gt_fname = 'Test_GroundTruth.csv'
...
task3_images_mask_npy_prefix = 'task3_image_masks'
task3_resized_images_npy_prefix = 'task3_resized_images'
task3_validation_images_npy_prefix = 'task3_validation_images'
task3_test_images_npy_prefix = 'task3_test_images'
task3_test_images_mask_npy_prefix = 'task3_test_masks'
...
def load_task3_training_images_seg(output_size=None):
    suffix = '' if output_size is None else '_%d' % output_size
    ...
    if os.path.exists(images_npy_filename) and os.path.exists(image_sizes_npy_filename):
        ...
        images, image_sizes = load_images(image_ids=task3_image_ids,
...
def load_task3_training_mask_images(output_size=None):
    suffix = '' if output_size is None else '_%d' % output_size
    images_npy_filename = os.path.join(cached_data_dir, '%s%s.npy' % (task3_images_mask_npy_prefix, suffix))

    if os.path.exists(images_npy_filename):
        images = np.load(images_npy_filename)
    else:
        images = load_images(image_ids=task3_image_ids,
                             from_dir=os.path.join(data_dir, 'Training_Part1_GroundTruth'),
                             output_size=output_size,
...
def load_task3_validation_mask_images(output_size=None):
    suffix = '' if output_size is None else '_%d' % output_size
    images_npy_filename = os.path.join(cached_data_dir, 'task3_validation_masks%s.npy' % suffix)

    if os.path.exists(images_npy_filename):
        images = np.load(images_npy_filename)
    else:
        images = load_images(image_ids=task3_image_ids,
                             from_dir=os.path.join(data_dir, 'Validation_Part1_GroundTruth'),
                             output_size=output_size,
                             fname_fn=lambda x: '%s_segmentation.png' % x)
        images = np.stack(images).astype(np.uint8)
        np.save(images_npy_filename, images)
...
def load_task3_test_mask_images(output_size=None):
    suffix = '' if output_size is None else '_%d' % output_size
    images_npy_filename = os.path.join(cached_data_dir, 'task3_test_masks%s.npy' % suffix)

    if os.path.exists(images_npy_filename):
        images = np.load(images_npy_filename)
    else:
        images = load_images(image_ids=task3_test_image_ids,
                             from_dir=os.path.join(data_dir, 'Test_Part1_GroundTruth'),
                             output_size=output_size,
                             fname_fn=lambda x: '%s_segmentation.png' % x)
        images = np.stack(images).astype(np.uint8)
        np.save(images_npy_filename, images)
...
def load_task3_resized_training_images(output_size=None):
    suffix = '' if output_size is None else '_%d' % output_size
    images_npy_filename = os.path.join(cached_data_dir, '%s%s.npy' % (task3_resized_images_npy_prefix, suffix))

    if os.path.exists(images_npy_filename):
        images = np.load(images_npy_filename)
    else:
        images = load_images(image_ids=task3_image_ids,
                             from_dir=task3_resized_img_dir,
                             output_size=output_size,
                             fname_fn=lambda x: '%s_resized.jpg' % x)
        images = np.stack(images).astype(np.uint8)
        np.save(images_npy_filename, images)
    return images

def load_task3_resized_validation_images(output_size=None):
    suffix = '' if output_size is None else '_%d' % output_size
    images_npy_filename = os.path.join(cached_data_dir, 'task3_validation_resized_images%s.npy' % suffix)

    if os.path.exists(images_npy_filename):
        images = np.load(images_npy_filename)
    else:
        images = load_images(image_ids=task3_validation_image_ids,
                             from_dir=os.path.join(data_dir, 'Validation_Resize'),
                             output_size=output_size,
                             fname_fn=lambda x: '%s_resized.jpg' % x)
        images = np.stack(images).astype(np.uint8)
        np.save(images_npy_filename, images)
...
def load_task3_resized_test_images(output_size=None):
    suffix = '' if output_size is None else '_%d' % output_size
    images_npy_filename = os.path.join(cached_data_dir, 'task3_test_resized_images%s.npy' % suffix)

    if os.path.exists(images_npy_filename):
        images = np.load(images_npy_filename)
    else:
        images = load_images(image_ids=task3_test_image_ids,
                             from_dir=task3_test_resized_img_dir,
                             output_size=output_size,
                             fname_fn=lambda x: '%s_resized.jpg' % x)
        images = np.stack(images).astype(np.uint8)
        np.save(images_npy_filename, images)
...
def load_task3_test_images_seg(output_size=None):
    suffix = '' if output_size is None else '_%d' % output_size
    images_npy_filename = os.path.join(cached_data_dir, '%s%s.npy' % (task3_test_images_npy_prefix, suffix))
    image_sizes_npy_filename = os.path.join(cached_data_dir, '%s_sizes.npy' % task3_test_images_npy_prefix)
    ...
    if os.path.exists(images_npy_filename) and os.path.exists(image_sizes_npy_filename):
        ...
        images, image_sizes = load_images(image_ids=task3_test_image_ids,
                                          from_dir=task3_test_img_dir,
                                          output_size=output_size,
                                          fname_fn=lambda x: '%s.jpg' % x, return_size=True)
        images = np.stack(images).astype(np.uint8)
...
def load_task3_training_masks(output_size=None):
    suffix = '' if output_size is None else '_%d' % output_size
    npy_filename = os.path.join(cached_data_dir, 'task3_masks%s.npy' % suffix)
    if os.path.exists(npy_filename):
        masks = np.load(npy_filename)
    else:
        masks = load_images(image_ids=task3_image_ids,
                            from_dir=os.path.join(data_dir, 'Training_Part1_GroundTruth'),
                            output_size=output_size,
                            fname_fn=lambda x: '%s_segmentation.png' % x)
        masks = np.stack(masks)
...
def load_task3_validation_masks(output_size=None):
    suffix = '' if output_size is None else '_%d' % output_size
    npy_filename = os.path.join(cached_data_dir, 'task3_val_masks%s.npy' % suffix)
    if os.path.exists(npy_filename):
        masks = np.load(npy_filename)
    else:
        masks = load_images(image_ids=task3_validation_image_ids,
                            from_dir=os.path.join(data_dir, 'Validation_Part1_GroundTruth'),
                            output_size=output_size,
                            fname_fn=lambda x: '%s_segmentation.png' % x)
        masks = np.stack(masks)
...
def load_task3_training_labels():
    ...
    labels = []
    with open(os.path.join(data_dir, task3_gt_fname), 'r') as f:
        for i, line in tqdm(enumerate(f.readlines()[1:])):
            fields = line.strip().split(',')
            # label = []
            labels.append([eval(field) for field in fields[1:]])
            # label.append(eval(fields[2]))
            # label.append(1-int(fields[2]))
            # labels.append(label)
        labels = np.stack(labels, axis=0)
    return labels

def load_task3_validation_labels():
    labels = []
    with open(os.path.join(data_dir, task3_val_gt_fname), 'r') as f:
        for i,line in tqdm(enumerate(f.readlines()[1:])):
            fields = line.strip().split(',')
            # label = []
            labels.append([eval(field) for field in fields[1:]])
...
def load_task3_test_labels():
    labels = []
    with open(os.path.join(data_dir, task3_test_gt_fname), 'r') as f:
        for i,line in tqdm(enumerate(f.readlines()[1:])):
            fields = line.strip().split(',')
            # label = []
            labels.append([eval(field) for field in fields[1:]])
...
def load_training_data(task_idx,
    ...
    # x = load_task3_resized_training_images(output_size = output_size)
    # y = load_task3_training_labels()
    # return partition_task3_data(x=x, y=y, k=num_partitions, i=idx_partition, test_split=test_split)
    return load_task3_resized_training_images(output_size=output_size), task3_image_ids, load_task3_training_labels()
...
def load_validation_data(task_idx, output_size=None):
    ...
    return load_task3_resized_validation_images(output_size=output_size), task3_validation_image_ids, load_task3_validation_labels()
...
def load_test_data(task_idx, output_size=None):
    ...
    # return load_task3_test_images(output_size=output_size), task3_test_image_ids, load_task3_test_labels()
    return load_task3_resized_test_images(output_size=output_size), task3_test_image_ids, load_task3_test_labels()


def load_training_seg_data(output_size=None):
    images, image_sizes = load_task3_training_images_seg(output_size=output_size)
    return images, task3_image_ids, image_sizes


def load_test_seg_data(output_size=None):
    images, image_sizes = load_task3_test_images_seg(output_size=output_size)
    return images, task3_test_image_ids, image_sizes
