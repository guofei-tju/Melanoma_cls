...
def print_precision_recall(precision, recall, f1, sp, labels):
    columnwidth = max([len(x) for x in labels] + [12])
    # Print header
    print()
    first_cell = " "
    ...
    # print f1
    print("|%{0}s|".format(columnwidth - 2) % 'f1 score', end="")
    for j in range(len(labels)):
        cell = "%{0}.3f|".format(columnwidth - 1) % f1[j]
        print(PrintColors.GREEN + cell + PrintColors.END_COLOR, end="")

    cell = "%{0}.3f|".format(columnwidth - 1) % np.mean(f1)
    print(PrintColors.BLUE + cell + PrintColors.END_COLOR, end="")

    print()

    # print sp
    print("|%{0}s|".format(columnwidth - 2) % 'sp', end="")
    for j in range(len(labels)):
        cell = "%{0}.3f|".format(columnwidth - 1) % sp[j]
        print(PrintColors.GREEN + cell + PrintColors.END_COLOR, end="")

    cell = "%{0}.3f|".format(columnwidth - 1) % np.mean(sp)
    print(PrintColors.BLUE + cell + PrintColors.END_COLOR, end="")
