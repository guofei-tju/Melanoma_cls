...
def get_specificity(y_true, y_pred):
    true_class = np.argmax(y_true, axis=1)
    pred_class = np.argmax(y_pred, axis=1)
    cnf = confusion_matrix(true_class, pred_class, labels=classes)

    specificity = np.zeros(len(classes))
    for k in range(len(classes)):
        tn = np.delete(cnf, [k], 0)
        tn = np.delete(tn, [k], 1)
        tn = np.sum(tn)
        fp = np.delete(cnf, [k], 0)
        fp = np.sum(fp[:, k])
...
def get_precision_recall(y_true, y_pred, print_pr=True):
    ...
    true_class = np.argmax(y_true, axis=1)
    pred_class = np.argmax(y_pred, axis=1)
    precision, recall, f1, _ = precision_recall_fscore_support(y_true=true_class,
                                                               y_pred=pred_class,
                                                               labels=classes,
                                                               warn_for=())
    specificity = get_specificity(y_true=y_true, y_pred=y_pred)
    if print_pr:
        print_precision_recall(precision=precision, recall=recall, f1=f1, sp=specificity, labels=class_names)
